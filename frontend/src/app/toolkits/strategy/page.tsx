      ...
      <div className="mx-auto max-w-6xl">
        <div className="relative overflow-hidden rounded-3xl border border-white/10 p-8 md:p-10 mb-8 bg-black/40">
          <div className="pointer-events-none absolute inset-0 -z-10">
            ...
            <div className="absolute -bottom-24 -right-20 h-72 w-72 rounded-full bg-gradient-to-tr from-indigo-500/20 to-violet-500/20 blur-3xl" />
          </div>
          <h1 className="text-4xl md:text-5xl font-black tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-rose-300 via-purple-300 to-indigo-300">Strategy & Motivations Toolkit</h1>
          <p className="text-lg md:text-xl text-gray-300 mt-3 max-w-2xl">Create strategic mappings and connect initiatives to outcomes.</p>
        </div>
        <section className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
          {links.map((l) => (
            <a key={l.label} href={l.href} className={`group relative rounded-2xl p-[1.5px] ${l.disabled ? 'opacity-60 cursor-not-allowed' : 'bg-gradient-to-br from-rose-500/40 to-purple-500/40 hover:from-rose-500 hover:to-purple-500 hover:scale-[1.01] transition-all duration-300'}`}>
